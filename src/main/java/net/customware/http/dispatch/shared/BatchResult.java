 ...
 * value, and there will be an exception available via the
 * {@link #getExceptions()} or {@link #getException(int)} methods
 * for the same index.
 * 
 * <p>
...
public class BatchResult implements Result, Iterable<Result>
{
	...
	/**
	 * Returns the result only if the value at the specified index is of the
	 * specified type.
	 * It does not support the specified class being a superclass of the result
	 * type.
	 * 
	 * @param <T>
	...
	public <T extends Result> T getResult(int i, Class<T> type)
	{
		...
		if (result != null
				&& type.getName().equals(result.getClass().getName()))
			return (T) result;
		return null;
	}
